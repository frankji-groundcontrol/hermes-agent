...
_LARK_SDK_IMPORTS = (
    ...
    ("lark_oapi.api.im.v1", (
        ...
        "CreateMessageRequest", "CreateMessageRequestBody", "GetChatRequest", "GetMessageRequest", "ListMessageRequest",
...
class FeishuAdapterSettings:
    ...
    allowed_group_chats: frozenset[str]
    ...
    reply_in_thread: bool = False  # accepted top-level group @mentions bootstrap a topic
...
class FeishuAdapter(BasePlatformAdapter):
    ...
    def _load_settings(extra: Dict[str, Any]) -> FeishuAdapterSettings:
        ...
        # See #86905.
        allow_bots = str(_get_scoped_secret("FEISHU_ALLOW_BOTS", extra.get("allow_bots", "none"))).strip().lower()
        ...
        return FeishuAdapterSettings(
            ...
            group_policy=str(_get_scoped_secret("FEISHU_GROUP_POLICY", extra.get("group_policy", "allowlist"))).strip().lower(),
            allowed_group_users=frozenset(_id_set(_get_scoped_secret("FEISHU_ALLOWED_USERS", "").split(","))),
            allowed_group_chats=parse_feishu_group_allowed_chats(_get_scoped_secret("FEISHU_GROUP_ALLOWED_CHATS", "")),
    ...
    async def _process_inbound_message(
        ...
        # root_id/parent_id also describe ordinary replies; only thread_id denotes a topic.
    ...
    def _allow_group_message(self, sender_id: Any, chat_id: str = "", *, is_bot: bool = False) -> bool:
        ...
        if policy == "disabled":
            return False
        # Listed rooms bypass sender/admin allowlists; bot admission and mention gating still apply.
        if chat_id in self._allowed_group_chats:
            return True
    ...
    async def _send_raw_message(
        ...
        reply_in_thread = bool(thread_id or route.get("reply_in_thread"))
        strict_thread = bool(thread_id or route.get("strict_thread"))
        effective_reply_to = reply_to or (route.get("reply_to_message_id") if reply_in_thread else None)
        if strict_thread and not effective_reply_to and not thread_id:
            raise RuntimeError("Feishu thread delivery requires a triggering message anchor")
        ...
        if not thread_id or self._response_succeeded(response) or str(getattr(response, "code", "")) != "99992402":
            return response
        # Public send rejects receive_id_type=thread_id (live verified); reply to an in-topic
        # anchor instead. Strict delivery never falls back to the main chat.
        anchor = await self._fetch_last_message_in_thread(thread_id)
        if anchor:
            logger.warning("[Feishu] thread_id receive rejected (code 99992402); replying into thread %s", thread_id)
            body = self._build_reply_message_body(
            ...
            return await self._run_blocking(self._client.im.v1.message.reply, request)
        logger.warning("[Feishu] thread_id receive rejected and no in-thread anchor found for %s; failing closed", thread_id)
        return response
...
def _apply_yaml_config(yaml_cfg: dict, feishu_cfg: dict) -> dict | None:
    """Translate config.yaml feishu: keys into adapter extras and env vars.

    Implements the apply_yaml_config_fn contract (#24849). Mirrors the legacy
    feishu_cfg block from gateway/config.py::load_gateway_config() (allow_bots).
    Env vars take precedence over YAML. Profile-scoped multiplex loads skip the
    legacy process-env bridge so one profile cannot seed another's bot policy.
    """
    if "allow_bots" not in feishu_cfg:
        return None

    allow_bots = str(feishu_cfg["allow_bots"]).lower()
    try:
        from agent.secret_scope import current_secret_scope, is_multiplex_active

        skip_env_bridge = bool(is_multiplex_active() and current_secret_scope() is not None)
    except Exception:
        skip_env_bridge = False
    if not skip_env_bridge and not os.getenv("FEISHU_ALLOW_BOTS"):
        os.environ["FEISHU_ALLOW_BOTS"] = allow_bots
    return {"allow_bots": allow_bots}
