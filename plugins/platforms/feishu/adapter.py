...
class FeishuAdapter(BasePlatformAdapter):
    ...
    def _load_settings(extra: Dict[str, Any]) -> FeishuAdapterSettings:
        ...
        allow_bots = str(
            _get_scoped_secret("FEISHU_ALLOW_BOTS", extra.get("allow_bots", "none"))
        ).strip().lower()
        ...
        return FeishuAdapterSettings(
            ...
            group_policy=str(
                _get_scoped_secret(
                    "FEISHU_GROUP_POLICY", extra.get("group_policy", "allowlist")
                )
            ).strip().lower(),
            allowed_group_users=frozenset(
                item.strip()
                for item in str(_get_scoped_secret("FEISHU_ALLOWED_USERS", "")).split(",")
                if item.strip()
            ),
            allowed_group_chats=parse_feishu_group_allowed_chats(
                _get_scoped_secret("FEISHU_GROUP_ALLOWED_CHATS", "")
            ),
            ...
            require_mention=_to_boolean(
                extra.get(
                    "require_mention",
                    _get_scoped_secret("FEISHU_REQUIRE_MENTION", "true"),
                )
            ),
            allow_all_users=_to_boolean(
                _get_scoped_secret("FEISHU_ALLOW_ALL_USERS", "false")
            ),
    ...
    def _apply_settings(self, settings: FeishuAdapterSettings) -> None:
        ...
        self._allowed_group_chats = settings.allowed_group_chats
