...
def handle_api_error(
    ...
    # One compatibility retry, after normal cleanup/classification/hooks. Never
    # discard output already streamed or mutate the canonical request history.
    _err_body = ""
    try:
        _err_body = str(getattr(api_error, "body", None) or getattr(api_error, "message", None) or api_error)
    except Exception:
        pass
    _wire_messages = api_kwargs.get("messages") if isinstance(api_kwargs, dict) else None
    if (
        not _retry.developer_role_retry_attempted
        and _retry.developer_role_retry_messages is None
        and status_code == 400
        and "developer is not one of" in _err_body.lower()
        ...
        and not _retry.api_output_started
    ):
        _retry.developer_role_retry_messages = [
            {**_wire_messages[0], "role": "system"}, *_wire_messages[1:],
        ]
        logger.info(
