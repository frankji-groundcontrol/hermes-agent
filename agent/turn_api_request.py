...
def build_api_request(
    ...
    api_start_time: Any, effective_task_id: Any, turn_id: Any, _retry: Any,
) -> ApiRequestBuild:
    """Assemble the attempt's request, then apply the compatibility wire override after
    middleware and before hooks/debug dumps observe the payload."""
    ...
    # Rewrite only the wire copy AFTER middleware; cached middleware payloads and
    # canonical api_messages retain the developer role for future requests.
    if _retry.developer_role_retry_messages is not None:
        api_kwargs = {**api_kwargs, "messages": _retry.developer_role_retry_messages}
        _retry.developer_role_retry_messages = None
        _retry.developer_role_retry_attempted = True
