...
class SessionPersistenceMixin:
    ...
    def _entry_locked(self, session_key: str) -> Optional[SessionEntry]:
        ...
        self._ensure_loaded_locked()
        return self._entries.get(self._resolve_session_key_locked(session_key))
    ...
    def _ensure_loaded_locked(self) -> None:
        ...
        # Partial route-move writes may leave an old key in one persistence leg.
        if self._reconcile_moved_routes_locked():
            try:
                self._save()
            except Exception as exc:
                logger.warning("gateway.session: moved-route reconciliation save failed: %s", exc)
    ...
    def _stale_entry_verdict(self, key: str, entry, row):
        ...
        if recovered_entry is not None and recovered_entry.session_id != entry.session_id:
            recovered_entry.metadata.update(self._preserved_routing_metadata(entry))
            logger.warning(
    ...
    def _reconcile_recovered_routing_locked(self) -> None:
        ...
        self._reconcile_moved_routes_locked()
