...
class BasePlatformAdapter(ABC):
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        session_key = self._resolve_inflight_session_alias(self._event_session_key(event))
    ...
    async def _handle_message_while_active(self, event: MessageEvent, session_key: str) -> None:
        ...
        if not cmd and event.allow_gateway_control:
            try:
                ...
                clarify_key = session_key
                profile = getattr(event.source, "profile", None) or getattr(self, "_gateway_profile_name", None)
                if profile and profile != "default" and session_key.startswith("agent:main:"):
                    clarify_key = f"agent:{profile}:{session_key.split(':', 2)[2]}"
                _has_text_clarify = _clarify_mod.get_pending_for_session(
                    clarify_key, include_choice_prompts=True) is not None
            except Exception:
    ...
    async def cancel_background_tasks(self) -> None:
        ...
        for bucket in (self._background_tasks, self._expected_cancelled_tasks, self._session_tasks,
                       ...
                       self._inflight_session_aliases):
            bucket.clear()
