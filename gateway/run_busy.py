...
class GatewayBusySessionMixin:
    ...
    def _enqueue_fifo(self, session_key: str, queued_event: "MessageEvent", adapter: Any) -> None:
        ...
        if pending_slot is None:
            return
        adapter_session_key = self._adapter_session_key_for_source(queued_event.source, session_key)
        if adapter_session_key in pending_slot:
            self._session_state(session_key).conversation.queued_events.append(queued_event)
        else:
            pending_slot[adapter_session_key] = queued_event
    ...
    def _promote_queued_event(
        ...
        *, adapter_session_key: Optional[str] = None,
        ...
        if adapter is not None and hasattr(adapter, "_pending_messages"):
            adapter._pending_messages[adapter_session_key or session_key] = overflow.pop(0)
        # else: no adapter — leave the head in place so we don't silently drop it.
        return pending_event

    def _queue_depth(self, session_key: str, *, adapter: Any = None, adapter_session_key: Optional[str] = None) -> int:
        """Total pending /queue items for a session — slot + overflow."""
        depth = len(self._overflow_queue(session_key) or ())
        if adapter is not None and (adapter_session_key or session_key) in getattr(adapter, "_pending_messages", {}):
            depth += 1
        return depth
    ...
    def _clear_goal_pending_continuations(self, session_key: str, adapter: Any, *, adapter_session_key: Optional[str] = None) -> int:
        """Remove queued synthetic /goal continuations for one session; real /queue items are kept."""
        removed = 0
        pending_slot = getattr(adapter, "_pending_messages", None) if adapter is not None else None
        if isinstance(pending_slot, dict):
            pending_event = pending_slot.get(adapter_session_key or session_key)
            if self._is_goal_continuation_event(pending_event):
                pending_slot.pop(adapter_session_key or session_key, None)
                removed += 1
    ...
    def _lookup_session_id_under_store_lock(session_store, session_key: str):
        ...
        with session_store._lock:  # noqa: SLF001
            ...
            resolver = getattr(type(session_store), "_resolve_session_key_locked", None)
            if callable(resolver):
                session_key = resolver(session_store, session_key)
    ...
    def _queue_or_replace_pending_event(self, session_key: str, event: MessageEvent) -> None:
        ...
        if not adapter:
            return
        adapter_session_key = self._adapter_session_key_for_source(event.source, session_key)
        ...
        existing = pending_slot.get(adapter_session_key) if isinstance(pending_slot, dict) else None
        ...
        if same_security_context and (
            ...
            merge_pending_message_event(
                adapter._pending_messages, adapter_session_key, event,
                merge_text=event.message_type == MessageType.TEXT,
            )
            event._gateway_accepted = True
            return

        if self._queue_depth(session_key, adapter=adapter, adapter_session_key=adapter_session_key) >= self._BUSY_QUEUE_MAX_PENDING:
            logger.warning(
    ...
    async def _busy_queue_command(self, event: MessageEvent, quick_key: str, source):
        ...
        depth = self._queue_depth(quick_key, adapter=adapter,
                                  adapter_session_key=self._adapter_session_key_for_source(source, quick_key))
