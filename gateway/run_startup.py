...
class GatewayStartupMixin:
    ...
    async def _run_startup_resume_event(
        ...
        try:
            ...
            task = session_tasks.get(self._adapter_session_key_for_source(event.source, session_key)) if isinstance(session_tasks, dict) else None
            if task is not None:
    ...
    def _start_check_access_policy(self) -> bool:
        ...
        from gateway.run import (
            _OWN_POLICY_OPEN_ENV, _own_policy_open_startup_violation, _warn_if_no_user_allowlist, _write_runtime_status_quiet
        )
        ...
        if _warn_if_no_user_allowlist(
            plugin_allowed_vars=tuple(allowed_vars), plugin_allow_all_vars=tuple(allow_all_vars)
        ):
            logger.warning(
