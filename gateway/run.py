...
def _warn_if_no_user_allowlist(
    *,
    ...
    any_allowlist = any(
        os.getenv(v) for v in GatewayStartupMixin._BUILTIN_ALLOWED_USERS_VARS + tuple(plugin_allowed_vars)
    ) or bool(parse_feishu_group_allowed_chats())
    allow_all = os.getenv("GATEWAY_ALLOW_ALL_USERS", "").lower() in {"true", "1", "yes"} or any(
        os.getenv(v, "").lower() in {"true", "1", "yes"}
        for v in GatewayStartupMixin._BUILTIN_ALLOW_ALL_VARS + tuple(plugin_allow_all_vars)
    )
...
class GatewayRunner(
    ...
    @staticmethod
    def _adapter_session_key_for_source(source: SessionSource, fallback: str) -> str:
        value = getattr(source, "__dict__", {}).get("_gateway_adapter_session_key")
        ...
        if hasattr(self, "session_store") and self.session_store is not None:
            try:
                ...
                if isinstance(session_key, str) and session_key:
                    adapter_key = self._adapter_session_key_for_source(source, "")
                    adapter_parts = adapter_key.split(":", 2)
                    if len(adapter_parts) == 3 and adapter_parts[0] == "agent":
                        namespace = session_key.split(":", 2)[:2]
                        return ":".join([*namespace, adapter_parts[2]])
                    return session_key
            except Exception:
                pass
    ...
    def _thread_metadata_for_source(
        ...
        if bootstrap_anchor:
            ...
            metadata.update({
                "reply_to_message_id": str(bootstrap_anchor),
                "thread_bootstrap_anchor_id": str(bootstrap_anchor),
