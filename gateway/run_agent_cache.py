    ...
    async def _interrupt_and_clear_session(
        ...
        if adapter and callable(interrupt_session_activity):
            ...
            if _accepts_keyword(interrupt_session_activity, "metadata"):
                await adapter.interrupt_session_activity(adapter_session_key, source.chat_id, metadata=metadata)
            else:
                await adapter.interrupt_session_activity(adapter_session_key, source.chat_id)
        if adapter and hasattr(adapter, "get_pending_message"):
            adapter.get_pending_message(adapter_session_key)  # consume and discard
        if state is not None:
