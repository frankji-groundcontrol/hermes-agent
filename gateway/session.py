...
from gateway.session_persistence import SessionPersistenceMixin, _DB_UNPINNED, _ROUTING_MOVED_FROM
...
class SessionSource:
    ...
    # Feishu top-level mention anchor is an om_* message, separate from the omt_* topic.
    ...
    _OPTIONAL_POST_SCOPE = (
        "parent_chat_id", "message_id", "reply_thread_anchor_id", "reply_thread_strict", "profile",
    )
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "SessionSource":
        plain = {
            ...
            if name not in {"chat_type", "reply_thread_strict"}
        }
        return cls(
            ...
            reply_thread_strict=bool(data.get("reply_thread_strict", False)), **plain,
        )
...
def is_shared_multi_user_session(
    ...
    return not (
        ...
        if source.thread_id or source.reply_thread_anchor_id else group_sessions_per_user
    )
...
def build_session_key(
    ...
    if not is_dm and not thread_id and source.reply_thread_anchor_id:
        # Preserve the group/channel slot until the provisional route moves to its real topic.
        thread_id = f"feishu-auto-{source.reply_thread_anchor_id}"
    if is_dm:
...
class SessionStore(
    ...
    def get_or_create_session(
        ...
        session_key = self._resolve_session_key(self._generate_session_key(source))
        ...
        try:
            slot.result = self._get_or_create_session_impl(
                source, force_new=force_new, touch_activity=touch_activity, session_key=session_key,
            )
            return slot.result
    ...
    def _get_or_create_session_impl(
        ...
        session_key = self._resolve_session_key(session_key or self._generate_session_key(source))
        now = _now()
        if not force_new:
    ...
    def _route_create(
        ...
        candidate = SessionEntry(
            ...
            metadata=self._preserved_routing_metadata(observed),
        )
        with self._lock:
            session_key = self._resolve_session_key_locked(session_key)
    ...
    def _replace_route_locked(self, session_key, old_entry, session_id, now, **fields) -> SessionEntry:
        ...
        new_entry = SessionEntry(
            ...
            metadata=self._preserved_routing_metadata(old_entry), **fields,
        )
