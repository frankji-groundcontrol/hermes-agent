...
function Set-PathVariable {
    ...
    # Also drop the in-checkout hermes-agent\bin PATH entry. Files stay on
    # disk so editor/ACP configs with absolute launcher paths keep working.
    if (-not $NoVenv) {
        $checkoutBin = "$InstallDir\bin"
        $items = @(($pathUpdate.Path -split ';') | Where-Object { $_ })
        if ($items -contains $checkoutBin) {
            $items = @($items | Where-Object { $_ -ne $checkoutBin })
            $pathUpdate.Path = $items -join ';'
            $pathUpdate.RemovedLegacy = $true
        }
    }
