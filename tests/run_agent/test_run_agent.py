...
def test_memory_provider_turn_start_precedes_prefetch_with_current_turn():
    from types import SimpleNamespace
    from agent.turn_context import _memory_turn_start_and_prefetch

    calls = []
    manager = SimpleNamespace(
        on_turn_start=lambda turn, query: calls.append(("start", turn, query)),
        prefetch_all=lambda query, session_id: calls.append(("prefetch", session_id, query)) or "",
    )
    agent = SimpleNamespace(_memory_manager=manager, _user_turn_count=7, session_id="session-test")
    query = "What did we decide about the deployment schedule?"

    assert _memory_turn_start_and_prefetch(agent, query) == ""
    assert calls == [("start", 7, query), ("prefetch", "session-test", query)]
