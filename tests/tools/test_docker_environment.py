    ...
    def _run(cmd, **kwargs):
        ...
        if isinstance(cmd, list) and len(cmd) >= 2:
            ...
            if sub == "ps":
                ...
                return subprocess.CompletedProcess(cmd, 0, stdout="", stderr="")
            if sub == "inspect" and ".Config.Labels" in " ".join(cmd):
                return subprocess.CompletedProcess(cmd, 0, stdout="0123456789abcdef01234567\n", stderr="")
            if sub == "run":
                return subprocess.CompletedProcess(cmd, 0, stdout="fresh-cid\n", stderr="")
        return subprocess.CompletedProcess(cmd, 0, stdout="", stderr="")
        ...
        if isinstance(cmd, list) and len(cmd) >= 2:
            ...
            if sub == "ps":
                ...
                if cmd[-1] == "{{.ID}}":
                    ...
                    return subprocess.CompletedProcess(
                        cmd, 0, stdout="same-cid\n", stderr=""
                    )
                return subprocess.CompletedProcess(cmd, 0, stdout="", stderr="")
            if sub == "inspect" and ".Config.Labels" in " ".join(cmd):
                return subprocess.CompletedProcess(cmd, 0, stdout=seen_spec["v"] + "\n", stderr="")
            if sub == "run":
                return subprocess.CompletedProcess(cmd, 0, stdout="fresh-cid\n", stderr="")
        return subprocess.CompletedProcess(cmd, 0, stdout="", stderr="")
