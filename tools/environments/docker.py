...
class DockerEnvironment(BaseEnvironment):
    ...
    def _attach_existing_container(self, task_label, profile_name, egress_label, network: bool) -> bool:
        ...
        existing = self._find_reusable_container(
            task_label, profile_name, egress_label, self._labels[_SPEC_LABEL_KEY])
        if existing is None:
            return False
        ...
        if not self._container_id:
            ...
            try:
                ...
                self._prune_superseded_containers(
                    self._labels.get("hermes-task-id", ""), self._labels.get("hermes-profile", ""),
                    self._labels.get(_SPEC_LABEL_KEY, ""))
    ...
    def _find_reusable_container(
        ...
        spec_label: str = "") -> Optional[tuple[str, str]]:
        ...
        if spec_label:
            filters.extend(["--filter", f"label={_SPEC_LABEL_KEY}={spec_label}"])
        result = _docker_query(
    ...
    def _prune_superseded_containers(self, task_label: str, profile_label: str, spec_label: str) -> None:
        """Remove same-slot containers with stale wiring after a fresh start; preserve host binds.
        Inspect the label separately: Podman does not support Docker's ps ``{{.Label}}`` method.
        Failed probes leave the container untouched.
        """
        listing = _docker_query(
            [self._docker_exe, "ps", "-a", "--filter", "label=hermes-agent=1",
             "--filter", f"label=hermes-task-id={task_label}",
             "--filter", f"label=hermes-profile={profile_label}", "--format", "{{.ID}}"],
            timeout=10, fail="Superseded-container probe failed: %s",
            nonzero="Superseded-container probe returned %d: %s")
        if listing is None:
            return
        for cid in (line.strip() for line in listing.stdout.splitlines() if line.strip()):
            if cid == self._container_id:
                continue
            old_spec = _docker_query(
                [self._docker_exe, "inspect", "--format", '{{index .Config.Labels "hermes-spec"}}', cid],
                timeout=10, fail="Superseded-container inspect failed: %s",
                nonzero="Superseded-container inspect returned %d: %s")
            if old_spec is None or old_spec.stdout.strip() == spec_label:
                continue
            logger.info("Removing superseded container %s (task=%s, profile=%s)",
                        cid[:12], task_label, profile_label)
            _docker_query(
                [self._docker_exe, "rm", "-f", cid], timeout=30,
                fail="Superseded-container removal failed: %s",
                nonzero="Superseded-container removal returned %d: %s")
