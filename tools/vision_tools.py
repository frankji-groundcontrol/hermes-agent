...
async def _vision_completion(call_kwargs: dict, vision_cfg: Optional[dict] = None):
    """Use the opted-in image transport on every attempt, then fall back to the shared router."""
    _load_auxiliary_client()
    if vision_cfg is not None:
        response = await _streamed_vision_completion(
            call_kwargs["messages"], vision_cfg=vision_cfg, timeout=call_kwargs["timeout"],
            temperature=call_kwargs["temperature"], model=call_kwargs.get("model"))
...
async def _call_vision_llm(
    call_kwargs: dict, empty_log: str, response=None, *, vision_cfg: Optional[dict] = None):
    ...
    _load_auxiliary_client()
    if response is None:
        response = await _vision_completion(call_kwargs, vision_cfg)
    analysis = extract_content_or_reasoning(response)
    if not analysis:
        logger.warning(empty_log)
        analysis = extract_content_or_reasoning(await _vision_completion(call_kwargs, vision_cfg))
    return analysis
...
async def vision_analyze_tool(
    ...
    async def stage(prompt: str, debug_call_data: dict, temp_paths: list) -> tuple:
        ...
        vision_cfg = _cfg_auxiliary("vision", default={}) or {}
        ...
        analysis = await _call_vision_llm(
            call_kwargs, "Vision LLM returned empty content, retrying once", response, vision_cfg=vision_cfg)
