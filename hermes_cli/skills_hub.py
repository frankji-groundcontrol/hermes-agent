...
def _inspect_local_skill(identifier: str) -> Optional[dict]:
    ...
    try:
        result = json.loads(skill_view(identifier, preprocess=False))
    except (TypeError, ValueError):
        return None
    if result.get("success"):
        return {
            "name": str(result.get("name") or identifier),
            "description": str(result.get("description") or "").strip(),
            "identifier": f"local/{identifier}",
            "content": str(result.get("content") or ""),
        }
    if result.get("matches") or "disabled" in str(result.get("error") or "").lower():
        return {
            "error": str(result.get("error") or f"Cannot inspect local skill '{identifier}'."),
...
def _print_local_skill(c: Console, skill: dict) -> None:
    ...
    if skill.get("error"):
        c.print(f"[bold red]Error:[/] {skill['error']}")
        return
    c.print()
    c.print(Panel(
        "\n".join([
